			...
			if(data.attr=='success'){
				...
				if(data.cet4list.length!=0){
					...
					for(var i=0;i<data.cet4list.length;i++){
						...
						' href="<%=basePath%>admin/cet_edit/'+data.cet4list[i].cet4Id+'"'+
					...
					if(data.attr=='success'){
						if(data.cet4list.length!=0){
							...
							for(var i=0;i<data.cet4list.length;i++){
								...
								'<i class="icon icon-edit i_green"></i> 编辑'+
								'</a>&nbsp;&nbsp;&nbsp;<a id="${list.cet4Id}" href="#"'+
								'onclick="cet_delete('+data.cet4list[i].cet4Id+')"> <i class="icon icon-trash i_red "></i>'+
								'删除</a></td></tr>';
								$("#show_cetinfo tbody").append(temp);
								$("input[name='page']").val("");
							}
